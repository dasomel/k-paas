...
> **권장사항**: 새 배포 시 [terraform-layered/](../terraform-layered/) 사용을 권장합니다.
> terraform-layered/는 고정 IP를 사용하여 LB Target 문제를 해결하고, 레이어별 독립적인 재배포가 가능합니다.
...
- **인프라**: Kakao Cloud (terraform-provider-kakaocloud v0.2.0)
...
# 특정 모듈 실행
terraform apply -target=module.provisioner -auto-approve
...
1. **Module: Network** (약 60분)
   - VPC 생성
   - Subnet 생성
   - Kakao Cloud API 응답 대기 시간 포함

2. **Module: Security** (약 5분)
   - Security Group 생성
   - 방화벽 규칙 설정

3. **Module: Compute** (약 15분)
   - Master 인스턴스 3개 생성
   - Worker 인스턴스 3개 생성
   - Public IP 할당
   - cloud-init 실행

4. **Module: LoadBalancer** (약 10분)
   - Master LB 생성 및 설정
   - Worker LB 생성 및 설정
   - Target Group 구성
   - Health Check 설정

5. **Module: Provisioner** (약 30분)
   - SSH 설정 및 호스트 등록
   - NFS 서버 설정
...
**총 예상 시간**: 약 2시간 (인프라 90분 + K-PaaS 설치 30분)

> **참고**: Kakao Cloud API 응답 시간이 느려 Network 모듈이 특히 오래 걸립니다.
...
- **[../docs/ARCHITECTURE.md](../docs/ARCHITECTURE.md)**: 상세 아키텍처 및 설계 문서
- **[../docs/README_POST_INSTALL_FIXES.md](../docs/README_POST_INSTALL_FIXES.md)**: 포스트 설치 수정 가이드
- **[../docs/SCRIPT_TEMPLATES.md](../docs/SCRIPT_TEMPLATES.md)**: 스크립트 템플릿
- **[../docs/POST_INSTALL_FIXES_SUMMARY.md](../docs/POST_INSTALL_FIXES_SUMMARY.md)**: 포스트 설치 요약
- **[../docs/k-paas.md](../docs/k-paas.md)**: K-PaaS 설치 요구사항
